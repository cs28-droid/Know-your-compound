...
int multibond=0;
int dashpos[100];// to store location of dashes before word root
...
//	initgraph(&gdriver, &gmode, "");
	...
	cin>>iupac;
	fstream fio;
	fio.open("compounds.txt", ios::in | ios::out | ios::app);
...
//Functional group structure//////////////////////////////
	...
	switch(choice){
		...
		case 2: system("cls");
				DecodeString();
				break;
		case 3: fio.open("compounds.txt", ios::trunc | ios::out);
				fio.close();
				system("cls");
...
int main()
{
	...
	for(int i=1;i<=40;i++)
	{
		cleardevice();
		setbkcolor(12);
		outtextxy(x,y-i,"WELCOME TO KNOW YOUR COMPOUND!");
		delay(100);
	}
	delay(500);
	settextstyle(6,0,4);
